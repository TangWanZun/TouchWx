...
Page({
   ...
   */
  chatSend(pro) {
    //如果当前用户已经离线，那么消息将无法发送
    if (!online){
      wx.showToast({
        title: '当前用户已离线，消息无法推送',
        icon:'none'
      })
      return
    }
    ...
    wx.$msgBroadcast.add('chat', function(msgList) {
      ...
      for (let i = 0; i < forList.length; i++) {
        ...
        if (index >= 0) {
          ...
          //当为fail为消息为失败
          if (item.MsgStatus == 'Fail') {
            state = _this.data.CHAT_CONST.LOG_FAIL
          }
          //添加并到消息列表
  ...
  onLoad: function(options) {
    ...
    //是否在线
    online = options.online == 'true' || options.online == 1
    //加载名称
    wx.setNavigationBarTitle({
      title: (online ? '' : '[离线]') + options.cardName
    })
    //加载数据
    this.getData(true);
    //获取登陆信息
